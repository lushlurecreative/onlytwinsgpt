...
export async function POST() {
  ...
  const handles = (String(handlesKey?.value ?? "")).split(",").map((h) => h.trim()).filter(Boolean);

  let imported = 0;
  if (handles.length > 0) {
    const items = await runActorAndGetItems(INSTAGRAM_ACTOR_ID, { usernames: handles });
