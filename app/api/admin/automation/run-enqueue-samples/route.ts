...
export async function POST() {
  ...
  if (!presetId) return NextResponse.json({ error: "Preset not found." }, { status: 500 });
  ...
  if (!qualified?.length) return NextResponse.json({ ok: true, enqueued: 0 });

  let enqueued = 0;
  for (const lead of qualified) {
    if (enqueued >= maxPerRun) break;
    const key = `lead_sample:${lead.id}`;
    const { data: existing } = await admin.from("idempotency_keys").select("key").eq("key", key).maybeSingle();
    ...
    const jobId = await createGenerationJob({
      subject_id: null, preset_id: presetId, reference_image_path: referenceImage,
      lora_model_reference: null, generation_request_id: null, job_type: "lead_sample", lead_id: lead.id,
    });
    if (!jobId) continue;
    ...
    await admin.from("automation_events").insert({
      event_type: "job_enqueued", entity_type: "lead", entity_id: lead.id,
      payload_json: { generation_job_id: jobId, source: "admin_override" },
    });
    enqueued++;
  }
